...
<template>
  <DialogRoot :open="open" @update:open="emit('update:open', $event)">
    <DialogPortal>
      ...
      <DialogContent class="form-dialog album-picker-dialog" :aria-describedby="undefined">
        <header><span class="form-dialog-icon"><FolderPlus :size="20"/></span><div><DialogTitle>添加到相册</DialogTitle></div><DialogClose aria-label="关闭"><X :size="20"/></DialogClose></header>
        <UiSelect v-if="albums.length" v-model="albumID" :options="options()" aria-label="目标相册"/>
        <p v-else class="section-note album-picker-empty">还没有相册</p>
