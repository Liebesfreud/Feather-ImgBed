...
async function saveSettings() {
  ...
  try {
    ...
    const random = { enabled: Boolean(latest.random?.enabled), album_id: latest.random?.album_id || '', tag_id: latest.random?.tag_id || '' }
    settings.value = await putJSON<Settings>('/settings', { ...settings.value, random })
    toast('基础设置已保存')
